...
We then a postprocessing step, \textttt{hstrat.PeelBackConjoinedLeavesTriePostprocessor} which accounts for the fact that entirely identical annotations may only arise due to spurious differentia collision because distinct leaf taxa by definition cannot have shared ancestry in their penultimate generation.
...
As our goal was to screen for possible effects, rather than establish the veracity of any one effext, we did not correct for multiple comparisons in assessing statistical significance.

...
One challenge encountered in these efforts, however, was in adapting hereditary stratigraphy algorithms to a lower-level, more resource-constrained programming environment.
The existing algorithms assumed a variable-size underlying data structure, which allows for growth in annotation size to provide hard guarantees on inference quality.
However, dynamic memory use under this approach entails several .
To overcome these challenges, second-generation applied ``surface'' approach, in contrast, gains by assuming a fixed-length buffer \citep{moreno2024algorithms}.
Section \ref{} discuss the distinctions 
